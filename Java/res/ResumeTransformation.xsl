<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
    ...
    <xsl:template match="/">
        ...
        <xsl:param name="arrayMonthsNames" select="document('')/*/xsl:variable[@name='inline-array']/*"/>
                                ...
                                <div>
                                    ...
                                    <div id="address-data" class="content-bg">
                                        ...
                                        <div class="center-txt">
                                            <xsl:value-of select="curriculum/address/street"/>&#160;<xsl:value-of
                                                select="curriculum/address/number"/>,&#160;<xsl:value-of
                                                select="curriculum/address/zipcode"/>,&#160;<xsl:value-of
                                                select="curriculum/address/city"/>,&#160;<xsl:value-of
                                                select="curriculum/address/state"/>
                                            <div class="spacebox2 content-bg"/>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div id="right-panel" class="grid-item">
                                ...
                                <table class="center-txt">
                                    ...
                                    <tbody>
                                        ...
                                        <xsl:for-each select="(//node())[$years >= position()-1]">
                                            ...
                                            <tr>
                                                <td>
                                                    <xsl:value-of select="substring($startDate, 1, 4)+position()-1"/>
                                                </td>

                                                <xsl:for-each select="(//node())[12 >= position()]">
                                                    <xsl:choose>
                                                        <xsl:when test="($iterator + position() = $startValue)">
                                                            <td class="highlight white-text">
                                                                <xsl:value-of select="substring($startDate,9,2)+0"/>
                                                            </td>
                                                        </xsl:when>
                                                        <xsl:when test="($iterator + position() = $endValue)">
                                                            <td class="highlight white-text">
                                                                <xsl:value-of select="substring($endDate,9,2)+0"/>
                                                            </td>
                                                        </xsl:when>
                                                        <xsl:when
                                                                test="($iterator + position() - 1 &gt;= $startValue) and ($iterator + position() - 1 &lt; $endValue)">
                            ...
                            <xsl:for-each select="curriculum/jobs/job">
                                <ul class="an-employment">
                                    ...
                                    <span class="bold">
                                        <xsl:variable name="mNumber" select="substring(startdate,6,2)+0"/>
                                        <xsl:value-of select="substring(startdate,9,2)"/> de <xsl:value-of select="$arrayMonthsNames[$mNumber]"/> del <xsl:value-of select="substring(startdate,1,4)"/><xsl:choose><xsl:when
                                            test=" $endDate = $currentDate"> al día de hoy
                                    </xsl:when><xsl:otherwise> — <xsl:variable name="mNumber2" select="substring(enddate,6,2)+0"/>
                                        <xsl:value-of select="substring(startdate,9,2)"/> de <xsl:value-of select="$arrayMonthsNames[$mNumber2]"/> del <xsl:value-of select="substring(enddate,1,4)"/></xsl:otherwise></xsl:choose></span>
                                    <ul>
                                    <li class="li-business">
                                        Empresa: <xsl:value-of select="company"/>
                                    </li>
                                    <li class="li-role">
                                        Puesto: <xsl:value-of select="position"/>
                                    </li></ul>
                                </ul>
                            </xsl:for-each>
